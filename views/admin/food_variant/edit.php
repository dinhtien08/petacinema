<div class="card p-4">
    ...
    <form action="?action=food_variant_editPost&id=<?= (int) $old['id'] ?>" method="post" class="row g-3" style="max-width: 500px;">

        <div class="col-12">
            <label class="form-label">Kích cỡ <span class="text-danger">*</span></label>
            <input type="text" name="size" class="form-control <?= !empty($errors['size']) ? 'is-invalid' : '' ?>" value="<?= old_value($old, 'size') ?>" maxlength="10">
        ...
        <div class="col-12">
            ...
            <a href="?action=food_variant_list&food_id=<?= (int) $old['food_id'] ?>" class="btn btn-outline-secondary">Hủy</a>
        </div>
