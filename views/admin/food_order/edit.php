<div class="card p-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <h4 class="mb-0">Sửa đơn món ăn #<?= (int) $old['id'] ?></h4>
    ...
    <p class="text-muted">Bước 1: Chọn món ăn &rarr; Bước 2: Chọn biến thể &rarr; Bước 3: Cập nhật thông tin đơn.</p>

    <?php if ($stage === 1): ?>

        <form action="?action=food_order_edit" method="get" class="row g-3" style="max-width: 500px;">
            <input type="hidden" name="action" value="food_order_edit">
            ...
            <div class="col-12">
                ...
                <select name="food_id" class="form-select">
                    ...
                    <?php foreach ($foods as $f): ?>
                        <option value="<?= (int) $f['id'] ?>" <?= (int) $old['food_id'] === (int) $f['id'] ? 'selected' : '' ?>><?= h($f['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <?php elseif ($stage === 2): ?>
        ...
        <p><strong>Món ăn đã chọn:</strong> <?= h($food['name']) ?>
            &nbsp;<a href="?action=food_order_edit&id=<?= (int) $old['id'] ?>">(Đổi món ăn)</a>
        </p>
                ...
                <div class="col-12">
                    ...
                    <select name="variant_id" class="form-select">
                        ...
                        <?php foreach ($variants as $v): ?>
                            <option value="<?= (int) $v['id'] ?>" <?= (int) $old['food_variant_id'] === (int) $v['id'] ? 'selected' : '' ?>><?= h($v['size']) ?> - <?= number_format((float) $v['price']) ?>đ (Tồn: <?= (int) $v['stock'] ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
    ...
    <?php else: ?>
        ...
        <p>
            ...
            &nbsp;<a href="?action=food_order_edit&id=<?= (int) $old['id'] ?>&food_id=<?= (int) $food['id'] ?>">(Đổi biến thể)</a>
        </p>

        <form action="?action=food_order_editPost&id=<?= (int) $old['id'] ?>" method="post" class="row g-3" style="max-width: 500px;">
